...
void sift_up(heap_t* heap, size_t pos){
    ...
    if(!(heap->condicion(heap->vector[pos_padre(pos)], heap->vector[pos]))){
        swap(heap->vector[pos], heap->vector[pos_padre(pos)]);
...
int hijo_incorrecto(heap_t* heap, size_t pos, heap_condicion condicion){
    ...
    void* hijo_derecho = heap->vector[pos_hijo_derecho(pos)];
    void* hijo_izquierdo = heap->vector[pos_hijo_izquierdo(pos)];
    ...
    if(!condicion(padre, hijo_derecho))
        return DERECHO;
    if(!condicion(padre, hijo_izquierdo))
        return IZQUIERDO;
    return NINGUNO;
}
...
void sift_down(heap_t* heap, size_t pos){
    ...
    if(hijo == DERECHO){
        swap(heap->vector[pos], heap->vector[pos_hijo_derecho(pos)]);
        sift_down(heap,pos_hijo_derecho(pos));
    }else if(hijo == IZQUIERDO){
        swap(heap->vector[pos], heap->vector[pos_hijo_izquierdo(pos)]);
...
int heap_borrar(heap_t* heap){
    ...
    if(heap->tope > 1){
        swap(heap->vector[RAIZ], heap->vector[heap->tope - 1]);
        void** aux = realloc(heap->vector, (sizeof(void*)*(heap->tope - 1)));
        if(!aux) return ERROR;
        heap->vector = aux;  
        heap->tope --;
